...
def _expand_combined_identity_row(
    ...
    *,
    ...
    return [
        ...
        f"| Address | {address_value} | {citation} |",
        f"| Owner | {owner_value} | {citation} |",
    ]
...
def _rebuild_summary_table_without_column_header(section: str) -> str:
    ...
    for index in table_indexes:
        ...
        if not cells or not cells[0] or set(cells[0]) == {"-"}:
            continue
        data_rows.append(lines[index])
    if not data_rows:
        return section
    rebuilt = [data_rows[0], "| --- | --- | --- |", *data_rows[1:]]
...
def _is_critical_current_position_line(line: str) -> bool:
    ...
    if normalized == "critical current position":
        return True
    if line.startswith("|"):
        first_cell = line.strip("|").split("|", maxsplit=1)[0].strip().casefold()
