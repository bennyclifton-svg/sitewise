...
_REGISTERS_FOOTER = "Registers to open: action, decision, risk, authority approvals, consultant appointment."
...
def _strip_narrative_placeholder(text: str) -> str:
    lines = [
        ...
        if NARRATIVE_PLACEHOLDER not in line
        and "Risk wording and owner decision due dates:" not in line
    ]
    return "\n".join(lines).strip()
...
def _merge_risks_section(scaffold: str, narrative: PmpNarrativeOutput) -> str:
    ...
    if narrative.risk_rows:
        risks = format_risk_rows_table(narrative.risk_rows)
    else:
        risks = _strip_registers_footer(_strip_narrative_placeholder(section))
    actions = format_register_rows_table(narrative.register_rows)
    replacement = f"### Risks\n\n{risks}\n\n### Actions and decisions\n\n{actions}"
    return _replace_markdown_section(
        scaffold, "Risks, decisions and next actions", replacement
    )
