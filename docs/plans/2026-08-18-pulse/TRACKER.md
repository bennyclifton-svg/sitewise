...
**Created:** 2026-08-18 · **Baseline commit:** `acb10131` · **Status:** in progress (Stage 0)
...
- [ ] 1.1 Failing test: `Cost Plan.pdf` keeps chunks
- [ ] 1.2 Add `has_useful_text` helper (D3 threshold)
- [ ] 1.3 `should_persist_chunks` keyed on text, not class
- [ ] 1.4 `_chunker_for` no longer keyed on `ingest_mode`
- [ ] 1.5 Narrow `_looks_like_drawing` `\bplan\b` (tactical safety fix)
- [ ] 1.6 Rewrite `tests/ingest/test_classify.py:55,67,94,114`
- [ ] 1.7 Drawing register still works (regression)
- [ ] 1.8 Full backend suite vs. baseline
...
| 2026-08-18 | Cursor Grok 4.6 | Census query 6 (`document_class` outside the current 18-value Literal) returned no rows. Ground-truth mentions `inbox_pending` and `corpus_catalog` as type violations; they are not present in this dev database snapshot. Stage 8 still has to handle the write sites. | open |
