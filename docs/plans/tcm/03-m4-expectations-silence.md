...
- [x] DSL evaluator: 100% comparator/combinator coverage, Appendix A examples pass verbatim
- [x] Downgrade test and never-auto-pass test exist and are green
- [x] Explicit-exclusion path provably LLM-free
- [ ] Full suite green (output pasted); silence eval baseline recorded
  - Partially complete: silence eval baseline recorded below. Latest full suite was not green: 490 passed, 1 skipped, 5 failed retrieval integration assertions, 3 warnings.
...
- [x] All analysis math covered by hand-computed table-driven tests
- [x] No English strings in analysis/silence modules (language only via report_language keys)
- [x] Integration fixture green; full suite green (output pasted)
...
- silence per-class P/R: offline smoke baseline (`SilencePredictionRunner` with fake adjudicator, no live model): bundled precision 1.0 / recall 1.0; ambiguous precision 1.0 / recall 1.0; excluded, ps_covered, and not_required unset because the smoke fixture has no observations for those classes.
