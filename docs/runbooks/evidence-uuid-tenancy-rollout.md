...
This runbook covers migrations 021, 021b, 022, and 023. Never use an application database for
...
4. Upgrade to `021b_source_doc_path_contract` to replace global path uniqueness
   with the project/platform partial indexes. This opens the bounded repair
   window; do not deploy a legacy single-path writer after this point.
5. Run `python scripts/repair_evidence_identity.py --dry-run`. Review counts.
6. Run `python scripts/repair_evidence_identity.py --apply` in a change window.
   Re-run it to verify idempotence, then re-run the audit. The unresolved
   ambiguous count must be zero.
7. Upgrade to `022_source_doc_uuid_contract`, then to
   `023_agent_turns`.
8. Verify two owners with the same slug and relative path through evidence API,
...
- Before 021b: roll the application back only to a version that tolerates the
...
- Downgrade 021b only while no cross-project duplicate paths exist. Migration
  021b intentionally refuses to restore global path uniqueness after repair;
...
- After 021b has restored global uniqueness, downgrade 021 only after confirming
  no application dependency on the nullable project UUID column.
