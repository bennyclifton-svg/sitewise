...
│   ├── scroll-mark.js  scroll-driven camera for landing pages
│   └── lighting.json   the rig as data, with its regression figures
├── landing-demo.html   working scroll-driven embed
└── logo-kit.html       every logo variant at every size
```
...
`3d/scroll-mark.js` flies the camera along the presets as the page
scrolls past. `landing-demo.html` is a working page; copy its markup.

```html
<section id="mark-track" style="height:400vh;position:relative">
  <div style="position:sticky;top:0;height:100vh;display:grid;place-items:center">
    <canvas id="mark-canvas" style="width:min(78vmin,720px);height:min(78vmin,720px)"></canvas>
  </div>
</section>
```
...
mountScrollMark(THREE, {
  ...
  OrbitControls   // optional — adds drag-to-orbit on top of the scroll path
});
```

The track's height is the camera path's length: 400vh gives four screens
of travel. The default path runs logo lock → three-quarter → plan → back
to logo lock; pass `path` to change it.

Two things it does that a naive version would not. The camera is
interpolated in **spherical** coordinates, not by lerping positions — a
straight lerp cuts a chord through the object and briefly puts the
camera inside it, whereas an arc holds a constant orbit, which is what
an architectural flythrough actually does. And scroll position is a
*target*, not the camera state: the camera eases toward it every frame,
so a trackpad flick glides instead of snapping.

Rendering pauses when the canvas leaves the viewport, and
`prefers-reduced-motion` drops the damping and the idle spin so the
camera tracks scroll exactly with no self-motion.
