...
export function mountLandingDemo(root, { playAll = false } = {}) {
  ...
  const state = { index: 0, timer: null, due: 0, remaining: 0, running: false, done: false, visible: false, userPaused: false, runId: 0, autoScrollTarget: null };
  ...
  function follow(element) {
    ...
    const viewport = documentView.getBoundingClientRect(), box = element.getBoundingClientRect();
    const clearance = sceneKey === 'program' ? 94 : sceneKey === 'cost' ? 98 : 18;
    if (box.bottom > viewport.bottom - 24 || box.top < viewport.top + clearance) {
      const top = Math.max(0, Math.min(documentView.scrollHeight - documentView.clientHeight, documentView.scrollTop + box.top - viewport.top - clearance));
  ...
  function complete() {
    ...
    find('[data-status]').textContent = `${scene.title} complete. ${sections.length} sections populated${sceneKey === 'program' ? ', with linked activities and milestones' : ', with source evidence'}.`;
  ...
  function buildSteps() {
    ...
    const actions = [
      ...
      ...sections.map((section, i) => ({ at: scanStart + i * 220, run: () => revealHeading(section) })),
  ...
  function pause() {
    ...
    root.getAnimations?.({ subtree: true }).forEach(animation => { if (animation.playState === 'running') animation.pause(); });
  ...
  function resume() {
    ...
    root.getAnimations?.({ subtree: true }).forEach(animation => { if (animation.playState === 'paused') animation.play(); });
    playButton.textContent = 'Pause scene';
    if (state.index === 0) find('.sw-changing-line').animate?.(
  ...
  function onManualScroll(event) {
    ...
    state.autoScrollTarget = null; state.userPaused = true; pause();
  }
