...
export function ComparisonList({ projectId }: { projectId: string }) {
  ...
  if (!comparisons.length) {
    return (
      <div className="flex min-h-72 items-center justify-center rounded-md border border-dashed bg-card p-6 text-center">
        <div className="max-w-sm">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Create a comparison through tender intake, then return here for QA,
            matrix, and report review.
          </p>
        </div>
      </div>
    );
